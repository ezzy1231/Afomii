    ...
    <div className="space-y-4">
      ...
      {signedIn !== false && myBookings.length > 0 && (
        <div className="rounded-xl border border-gold/30 bg-gold/5 p-5">
          <h3 className="flex items-center gap-2 font-serif text-lg font-bold text-app-fg">
              ...
              >
                <div className="text-sm">
                  <span className="font-semibold text-app-fg">
                    {new Date(`${b.reservation_date}T00:00:00`).toLocaleDateString('en-GB', {
                      weekday: 'short',
                      day: '2-digit',
                      month: 'short',
                    })}
                  </span>
                  <span className="tabular-nums text-app-muted"> · {b.time_slot}</span>
      ...
      {result?.ok ? (
        <div className="rounded-xl border border-app-border bg-app-card p-5">
          ...
          <p className="mt-2 text-xs font-medium text-gold-soft">
            {branch.branchName} · {date} · {timeSlot} · {partySize} guests — it now appears in
            &ldquo;Your reservations&rdquo; above and under Settings.
          </p>
          <button
            type="button"
        ...
        >
          ...
          <div className="mt-4 space-y-4">
            ...
            {result && !result.ok && (
              <p className="rounded-md bg-danger/10 px-3 py-2 text-xs text-danger">{result.message}</p>
            )}
